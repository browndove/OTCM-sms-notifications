...
async function getAllArkeselIds(campaignId = null) {
  await ensureSchema();
  if (campaignId) {
    const { rows } = await getPool().query(
      `SELECT DISTINCT arkesel_id FROM messages
       WHERE arkesel_id IS NOT NULL AND campaign_id = $1`,
      [campaignId]
    );
...
async function getSmsReportStats(campaignId = null) {
  await ensureSchema();
  if (campaignId) {
    const { rows } = await getPool().query(
      `SELECT
        ...
        COUNT(*) FILTER (WHERE r.status NOT IN ('DELIVERED', 'SUBMITTED'))::int AS failed,
        COALESCE(SUM(r.units), 0)::int AS units_used
      FROM sms_reports r
      INNER JOIN messages m ON m.id = r.message_id
      WHERE m.campaign_id = $1`,
      [campaignId]
