...
async function syncReportsFromArkesel(db, arkesel, { campaignId = null } = {}) {
  const arkeselIds = await db.getAllArkeselIds(campaignId);
  if (!arkeselIds.length) {
    return { synced: 0, updated: 0, arkeselIds: 0, campaignId };
  }
  ...
  return { synced, updated, arkeselIds: arkeselIds.length, campaignId };
}
