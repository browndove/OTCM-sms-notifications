...
/** Fetch delivery reports for up to 1000 message UUIDs per call. */
async function getMessageReports(msgIds) {
  const api = reportsClient();
  const { data } = await api.post('/sms/message-reports', { msg_ids: msgIds });
  return data;
}

module.exports = { sendSms, getSmsStatus, getBalance, getMessageReports };
